\section{What is Monte Carlo Neutron Transport?}

%1.	Background in Monte Carlo Transport - Physics / "what is" papers
%	
%	a. Types of transport ( Neutron, Photon, Charged Particle )
%	
%	b. fundamental methods ( varience reduction, nuclear data lookups )
...
E. E. Lewis and W. F. Miller Jr. in Computational Methods of Neuron Transport describes Monte Carlo transport as a simulation of some number of particle histories by using a random number generator.
%
...
%
This equation is known as the Linearized Boltzmann transport equation for neutrons:
%
$$
...
While there are many ways people can solve this problem generally people consider tracking particle histories and the algorithm that follows is often called the history based approach because of this.
% 
Algorithm~\ref{alg:historybased} shows the history based approach for a simple research code~\cite{alpsmc1} but has the same properties we can use to describe the method.
%
...
%
For example in Algorithm~\ref{alg:historybased} this takes place inside the Cycle Loop and shows only the steps for Cycle Tracking.
...
%Main Phases
%%
%Init( Initializing data structures / load balancing )
%%
%Tracking ( do particle tracking physics )
%%
%Finalize ( reduce tallies / collect statistics )
%%
%Variance Reductions( can occure in init or tracking )
%%
%
%%
